...
r_make_glyph(R_Font *font, stbtt_fontinfo stb_font, U32 stb_glyph_index, U32 glyph_array_index, U32 codepoint, R_FontRenderMode r_mode, F32 scale)
{
    ...
    if (glyph_dim.x > 0 && glyph_dim.y > 0)
    {
        ...
        stbtt_MakeGlyphBitmap(&stb_font, glyph_bitmap + glyph_offset_x, padded_glyph_dim.x * (S32) horizontal_resolution, padded_glyph_dim.y, (int) bitmap_stride, scale * (F32) horizontal_resolution, scale, (int) stb_glyph_index);
        ...
        RectU32 adjusted_rect_region =
            {
                .min = rect_region.min,
                .max = v2u32(rect_region.min.x + (U32) padded_glyph_dim.x, rect_region.min.y + (U32) padded_glyph_dim.y),
            };
    ...
    glyph->bearing_in_pixels = v2f32(f32_round((F32) glyph_left_side_bearing * scale), f32_floor((F32) glyph_bounding_box.y1 * scale));
