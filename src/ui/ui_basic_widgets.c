...
ui_combo_box_internal(Str8 name, U32 *selected_index, Str8 *item_names, U32 item_count, UI_ComboBoxParams *params)
{
    ...
    if (params->item_size.kind == UI_SizeKind_Null)
    {
        ...
        {
            Vec2F32 size  = d_measure_text(font, item_names[i]);
...
ui_get_codepoint_index_from_mouse_pos(UI_Box *box, Str8 edit_str)
{
    ...
    for (U64 i = 0; i < edit_str.size;)
    {
        ...
        Vec2F32 dim            = d_measure_character(r_font_from_key(ui_top_font_key()), decode.codepoint);
...
ui_line_edit(UI_TextEditState *edit_state, U8 *buffer, U64 buffer_size, U64 *string_length, Str8 string)
{
    ...
    ui_seed(string)
    {
        ...
        if (ui_box_is_focused(box))
        {
            ...
            ui_parent(box)
            {
                Vec2F32 cursor_offset   = d_measure_text_length(r_font_from_key(ui_top_font_key()), buffer_str8, (U64) edit_state->cursor);
                ...
                {
                    Vec2F32 mark_offset = d_measure_text_length(r_font_from_key(ui_top_font_key()), buffer_str8, (U64) edit_state->mark);
                ...
                Vec2F32 text_size = d_measure_text(r_font_from_key(ui_top_font_key()), edit_str);
